...
performs the logic for requests in separate processes and uses a local NVIDIA
Triton model server to handle the model.
"""
...
def main(argv: Sequence[str]) -> None:
  ...
  # with open(
  #     os.path.join(local_path, 'vertex_schemata', 'response.yaml'), 'r'
  # ) as f:
  #   prediction_validator = jsonschema.Draft202012Validator(yaml.safe_load(f))
  prediction_validator = None
  predictor_instance = predictor.MedASRPredictor(
      model_source=os.environ.get('MODEL_FILES')
  )
